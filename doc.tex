...
Naive推定量は、
\begin{equation}
    \hat{V}_{\text{Naive}}(\pi;\mathcal{D}) = \sum_{i = 1}^{n}\sum_{k = 1}^{K} \frac{\pi(S_{i}(k)|x_{u_{i}}, k)}{\sum_{j = 1}^{n}\pi(S_{j}(k)|x_{u_{j}}, k)} C_{i}(k)R_{i}(k)
\end{equation}
で定義される推定量である。
...
DM推定量は、
\begin{equation}
    \hat{V}_{\text{DM}}(\pi;\mathcal{D}) = \frac{1}{n} \sum_{i = 1}^{n} \sum_{k = 1}^{K} \mathbb{E}_{\pi(S|x_{u_{i}})}\left[q(x_{u_{i}}, e_{S(k)})\right]
\end{equation}
で定義される推定量である。ここで、
\begin{equation}
    q(x_{u_{i}}, e_{S(k)}) = \mathbb{E}\left[C(k)R(k) | x_{u}, e_{S(k)}\right]
\end{equation}
である。すなわち、$q(x_{u_{i}}, e_{S(k)})$はユーザー$u$がアイテム$S(k)$をCVする確率である。
